...
PUBLIC_URL = 'http://127.0.0.1:5500'
REDIRECT_URI = ""
...
def login():
    ...
    if tokens == None or time.time() > (tokens['expires_at']+(tokens['refresh_token_expires_in']-tokens['expires_in'])):
        ...
        teams = OAuth2Session(os.getenv('CLIENT_ID'),
                              scope=SCOPE, redirect_uri=REDIRECT_URI)
        authorization_url, state = teams.authorization_url(
            AUTHORIZATION_BASE_URL)

        # State is used to prevent CSRF, keep this for later.
        print("Storing state: ", state)
        session['oauth_state'] = state
        print("root route is re-directing to ", authorization_url,
...
def callback():
    ...
    print("Calling OAuth2SEssion with CLIENT_ID ", os.getenv('CLIENT_ID'),
          " state ", session['oauth_state'], " and REDIRECT_URI as above...")
    auth_code = OAuth2Session(os.getenv(
        'CLIENT_ID'), state=session['oauth_state'], redirect_uri=REDIRECT_URI)
    print("Obtained auth_code: ", auth_code)
    print("fetching token with TOKEN_URL ", TOKEN_URL, " and client secret ",
          os.getenv('CLIENT_SECRET'), " and auth response ", request.url)
...
def started():
    ...
    return ("""<!DOCTYPE html>
               <html lang="en">
                   <head>
                       <meta charset="UTF-8">
                       <title>Webex oAuth and refresh sample</title>
                   </head>
               <body>
               <p>
...
def webex_teams_webhook_refresh():
    ...
    print("Current time: ", time.time(),
          " Token expires at: ", access_token_expires_at)
